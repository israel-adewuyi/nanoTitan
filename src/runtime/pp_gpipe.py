...
class GPipePipelineParallel(Runtime):
    ...
    def train_step(self, model, batch, optimizer):
        ...
        for mb_x, mb_y in zip(microbatch_x, microbatch_y, strict=False):
            if self.is_main_rank():
                ...
                logger.debug(
                    f"At rank {self.rank}!!! Receiving activations from rank {self.prev_rank}"
                )
    ...
    def backward(self, loss, model, microbatch_idx):
        ...
        if not self.is_main_rank():
            dist.send(model.get_incoming_acts_grad(microbatch_idx), self.prev_rank)

        if microbatch_idx == 0 and (self.is_main_rank() or self.is_last_rank):
